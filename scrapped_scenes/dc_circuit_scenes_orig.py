from accalib.svg_objects import *
...
class SimpleDCCircuit(Scene):
    ...
    def get_circuit(self):
        ...
        voltage_source.shift(0.7*DOWN+1.2*RIGHT)
        vs_text = TextMobject("$12V$")
        vs_text.scale(1.75)
        vs_text.next_to(voltage_source,direction=LEFT,buff=0.1)
        resistor = Resistor()
        resistor.scale(1.25)
        resistor.next_to(voltage_source,direction=RIGHT,buff=1.7)
        r_text = TextMobject("$2\Omega$")
        r_text.scale(1.75)
        r_text.next_to(resistor,direction=RIGHT,buff=0.4)
        wires = []
