...
class MyDecimalNumber(VMobject):
    CONFIG = {
    ...
    def __init__(self, number=0, **kwargs):
        ...
        if self.use_my_alignment:
            self.unit_sign[0].next_to(self.submobjects[-2], direction=RIGHT, buff=self.digit_to_digit_buff)
            for i in range(1, len(self.unit_sign)):
                self.unit_sign[i].next_to(self.unit_sign[i - 1], direction=RIGHT, buff=self.digit_to_digit_buff,
                                          aligned_edge=DOWN)
            self.unit_sign[2].next_to(self.unit_sign[3], direction=LEFT, aligned_edge=UP, buff=self.digit_to_digit_buff)
    ...
    def set_value(self, number, **config):
        ...
        new_decimal = MyDecimalNumber(number, **full_config)
